...
# Keeping the data columns that we require
...
# Creating a time series dataframe for the LSTM model
...
# Creating the input vector and the output vector
X = shifted_df_as_np[:, 1:]
y = shifted_df_as_np[:, 0]

# Flipping the dataset so that the model gets t-7 days first and so on
...
# Training loop with scheduler stepping
for epoch in range(num_epochs):
    train_one_epoch()

    # Get the validation loss to pass to the scheduler
...
plt.plot(new_y_train, label='Actual Close (Train)', color='blue')
plt.plot(train_predictions, label='Predicted Close (Train)', color='orange')
...
plt.plot(new_y_test, label='Actual Close (Test)', color='blue')
plt.plot(test_predictions, label='Predicted Close (Test)', color='orange')
